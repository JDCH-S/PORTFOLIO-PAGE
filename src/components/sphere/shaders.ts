...
export const fragmentsVert = /* glsl */ `
  ...
  attribute vec3 iTangent;  // unit tangent (perpendicular to iCenter): arc direction
  attribute vec4 iParams;   // radius, arc length (rad), width (world), seed
  ...
  void main() {
    ...
    vec3 c = iCenter;
    vec3 tg = iTangent;
    vec3 p = c * cos(a) + tg * sin(a);       // point on the great circle
    vec3 tp = -c * sin(a) + tg * cos(a);     // tangent at p
...
export const fragmentsFrag = /* glsl */ `
  ...
  void main() {
    ...
    if (vType < 0.5) {                              // arc: tapered ends
      ...
      across = soft;
    } else if (vType < 2.5) {                       // shard: hard rectangle with faint halo
      along = smoothstep(0.0, 0.03, t) * smoothstep(1.0, 0.97, t);
      across = step(s, 0.8) * 0.85 + 0.15 * soft;
...
export const vortexVert = /* glsl */ `
  ...
  vec3 spiral(float t, float seed, float phase, float rs) {
    if (iKind > 0.5) {
      // closed bright ring, tilted per instance, slowly counter-rotating
      float rr = uRadius * rs * (0.2 + 0.36 * seed);
...
export const vortexFrag = /* glsl */ `
  ...
  void main() {
    ...
    float soft = 1.0 - s * s;
    soft *= soft;
    float along = smoothstep(0.0, 0.18, t);
    vec3 col = mix(uColorBase, uColorHot, smoothstep(0.35, 1.0, t));
